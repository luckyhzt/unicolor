...
if __name__ == '__main__':
    ...
    # Sampling arguments
    img_size = [256, 256]
    image_path = 'C:\\Users\\lucky\\Desktop\\visual\\stroke'
    save_dir = 'C:\\Users\\lucky\\Desktop\\visual\\stroke\\ideep'
    num_strokes = [2, 16]
    ...
    for i in range (6, 7):
        with open(os.path.join(image_path, str(i), 'strokes.json'), 'r') as file:
            strokes = json.load(file)

        gen_imgs = []

        I_color = Image.open(os.path.join(image_path, str(i), 'gray.png')).convert('RGB')
        ...
        colorModel.load_image(os.path.join(image_path, str(i), 'gray.png')) # load an image
