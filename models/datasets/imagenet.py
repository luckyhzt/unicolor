...
class Imagenet(Data.Dataset):
    def __init__(self, datapath, split, color, resolution, superpixel=None, patch_size=None, mode_prob=None, strokes=None, threshold=None):
        super().__init__()
    ...
    def loadInfo(self):
        with open(os.path.join(self.datapath, f'imagenet_colorfulness_{self.split}.json'), 'r') as f:
            infos = json.load(f)
        ...
        if self.threshold == None:
            ...
            self.infos = []
            for file in infos:
                if file['colorfulness'] >= self.threshold:
                    self.infos.append(file)


    def __getitem__(self, index):
        img_path = os.path.join(self.datapath, self.split, self.infos[index]['path'])
        image = Image.open(img_path).convert('RGB')
        if self.superpixel != None:
            sp_path = os.path.join(self.datapath, f'{self.split}_{self.superpixel}', self.infos[index]['path'].replace('.JPEG', '.png'))
